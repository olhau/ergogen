module.exports = {
  ...
  body: p => {
    ...
    let traces = '';
    if (p.include_traces) {
      // --- D+ Routing (Connects to Via at Y = -4.0) ---
      // Front D+
      traces += seg(-0.75, -2.225, -0.25, -2.225, 'F.Cu'); // Bridge pads A6/B7
      traces += seg(-0.5, -2.225, -0.5, -4.0, 'F.Cu');     // Route up
      traces += seg(-0.5, -4.0, 0, -4.0, 'F.Cu');          // Connect to via
      // Back D+
      traces += seg(0.25, -2.225, 0.75, -2.225, 'B.Cu');   // Bridge back pads
      traces += seg(0.5, -2.225, 0.5, -4.0, 'B.Cu');
      traces += seg(0.5, -4.0, 0, -4.0, 'B.Cu');

      // --- D- Routing (Connects to Via at Y = -5.5) ---
      // Front D-
      traces += seg(0.25, -2.225, 0.75, -2.225, 'F.Cu');   // Bridge pads A7/B6
      traces += seg(0.5, -2.225, 0.5, -5.5, 'F.Cu');       // Route up
      traces += seg(0.5, -5.5, 0, -5.5, 'F.Cu');           // Connect to via
      // Back D-
      traces += seg(-0.75, -2.225, -0.25, -2.225, 'B.Cu'); // Bridge back pads
      traces += seg(-0.5, -2.225, -0.5, -5.5, 'B.Cu');
      traces += seg(-0.5, -5.5, 0, -5.5, 'B.Cu');

      // --- GND Routing (Connects to Via at Y = -7.0, Thicker 0.4mm traces) ---
      // Front GND Left
    ...
    (footprint "USB-C-SHOU_TYPE-C-16P-REVERSIBLE-ROUTED" (layer "${p.side}.Cu")
      ...
      ;; Silkscreen Via Labels for User
      ...
      ;; ── COURTYARD (Expanded to Y = -8.0 to protect via routing) ──────────
      ...
      ;; ── EXPOSED OUTPUT VIAS (Where user starts routing) ──────────────────
    ...
    ;; ── INJECTED NATIVE TRACES ─────────────────────────────────────────────
    ;; These are injected immediately after the footprint S-expression finishes.
${traces}
    `;
  }
};
