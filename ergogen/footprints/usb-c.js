// Ergogen footprint: USB-C-SHOU TYPE-C-16P-REVERSIBLE
// LCSC C2906290 — mid-mount, reversible, GND + D+/D- only.
//
// Dimensions are based on physical measurements of the actual component
// (Aliexpress variant — may differ slightly from the LCSC datasheet):
//   Overall width (outer shield tab to outer shield tab): 11.50 mm
//   GND outer tab span (A1/A12 outer edge to outer edge):  6.78 mm
//     → pad centres at ±3.29 mm (assuming ~0.20 mm tab width)
//   Signal tab protrusion beyond connector body:           0.77 mm
//   Shield mounting tab cross-section:
//     width (all 4):                                       0.30 mm
//     length — rear pair:                                  1.21 mm
//     length — front pair:                                 0.80 mm
//
// This connector is always soldered on both F.Cu and B.Cu simultaneously
// (mid-mount reversible), so pads appear on both copper layers regardless
// of placement side.
//
// Net params:
//   GND    — pins A1, B1, A12, B12 (corner GND) + S1 shield tabs
//   DPLUS  — pins A6 and B6 (USB-C spec: D1+ and D2+)
//   DMINUS — pins A7 and B7 (USB-C spec: D1- and D2-)
//
// USB-C spec (IEC 62680-1-3) table row for signal pins:
//   A6 (D1+) | A6 | B7 | D2- (B7)   ← A6 and B7 face each other across the slot
//   A7 (D1-) | A7 | B6 | D2+ (B6)   ← A7 and B6 face each other across the slot
// So A6 = D+, B6 = D+, A7 = D-, B7 = D-.
// The previous version had B6 and B7 swapped, which would invert D+/D- in
// one of the two plug orientations.
//
// Rotation note: all rect pads and oval thru-hole pads include ${p.r}
// in their (at ...) so KiCad rotates the pad shape correctly when the
// footprint is placed at a non-zero angle.
//
// Pad geometry note:
//   The physical connector housing lip extends roughly 0.9 mm past the
//   connector body outline, covering the inner half of the original 1.8 mm
//   pads and leaving only ~0.9 mm of solderable land — marginal for drag
//   soldering by hand.
//
//   Fix: the pad inner edge (connector side) is kept at its original
//   position; the outer edge is extended further into the PCB to give
//   ~2 mm of exposed land.  Concretely:
//     • pad height:  1.8 mm  →  3.0 mm
//     • pad center Y: -2.225  →  -2.825   (shift of -0.6 mm)
//   The courtyard top edge is updated to -4.5 to encompass the new pads.

module.exports = {
  params: {
    designator: 'USB',
    GND:    { type: 'net', value: 'GND' },
    DPLUS:  { type: 'net', value: 'D+' },
    DMINUS: { type: 'net', value: 'D-' },
  },

  body: p => {
    // p.NET.str is the complete KiCad (net INDEX "NAME") string ergogen provides
    const gnd = p.GND.str
    const dp  = p.DPLUS.str
    const dm  = p.DMINUS.str

    return `
    (footprint "USB-C-SHOU_TYPE-C-16P-REVERSIBLE"
      (layer "F.Cu")
      ${p.at}
      (descr "Mid-mount USB-C, SHOU TYPE-C-16P-CB1.6, LCSC C2906290. GND+D+/D- only, reversible F.Cu+B.Cu, extended pads for drag soldering.")
      (tags "USB-C mid-mount reversible")

      (property "Reference" "${p.ref}"
        (at 0 3.5)
        (layer "F.Fab")
        (effects (font (size 1 1) (thickness 0.15)))
      )
      (property "Value" "USB-C-SHOU_TYPE-C-16P-REVERSIBLE"
        (at 0 5.5)
        (layer "F.Fab")
        (effects (font (size 1 1) (thickness 0.15)))
      )

      ${''/* ── Outline / courtyard ───────────────────────────────── */}

      (fp_line (start -4.47 -1.82) (end 4.47 -1.82) (layer "Cmts.User") (width 0.25))
      (fp_line (start  4.47 -1.82) (end 4.47  4.78) (layer "Cmts.User") (width 0.25))
      (fp_line (start -4.47 -1.82) (end -4.47 4.78) (layer "Cmts.User") (width 0.25))

      (fp_line (start -4.47 -1.83) (end  4.47 -1.83) (layer "F.SilkS") (width 0.25))
      (fp_line (start  4.47 -1.83) (end  4.47  4.67) (layer "F.SilkS") (width 0.25))
      (fp_line (start  4.47  4.67) (end -4.47  4.67) (layer "F.SilkS") (width 0.25))
      (fp_line (start -4.47  4.67) (end -4.47 -1.83) (layer "F.SilkS") (width 0.25))

      (fp_line (start -4.47 -1.83) (end  4.47 -1.83) (layer "B.SilkS") (width 0.25))
      (fp_line (start  4.47 -1.83) (end  4.47  4.67) (layer "B.SilkS") (width 0.25))
      (fp_line (start  4.47  4.67) (end -4.47  4.67) (layer "B.SilkS") (width 0.25))
      (fp_line (start -4.47  4.67) (end -4.47 -1.83) (layer "B.SilkS") (width 0.25))

      (fp_line (start -4.47 -1.83) (end  4.47 -1.83) (layer "F.Fab") (width 0.10))
      (fp_line (start  4.47 -1.83) (end  4.47  4.67) (layer "F.Fab") (width 0.10))
      (fp_line (start  4.47  4.67) (end -4.47  4.67) (layer "F.Fab") (width 0.10))
      (fp_line (start -4.47  4.67) (end -4.47 -1.83) (layer "F.Fab") (width 0.10))

      ${''/* Courtyard X extended to ±6.4 to encompass shield pads (outer edge  */}
      ${''/* at ±5.62 + 0.4 pad half-width = ±6.02, +0.25 clearance = ±6.27). */}
      (fp_line (start -6.40 -4.50) (end  6.40 -4.50) (layer "F.CrtYd") (width 0.05))
      (fp_line (start  6.40 -4.50) (end  6.40  4.68) (layer "F.CrtYd") (width 0.05))
      (fp_line (start  6.40  4.68) (end -6.40  4.68) (layer "F.CrtYd") (width 0.05))
      (fp_line (start -6.40  4.68) (end -6.40 -4.50) (layer "F.CrtYd") (width 0.05))

      (fp_line (start -6.40 -4.50) (end  6.40 -4.50) (layer "B.CrtYd") (width 0.05))
      (fp_line (start  6.40 -4.50) (end  6.40  4.68) (layer "B.CrtYd") (width 0.05))
      (fp_line (start  6.40  4.68) (end -6.40  4.68) (layer "B.CrtYd") (width 0.05))
      (fp_line (start -6.40  4.68) (end -6.40 -4.50) (layer "B.CrtYd") (width 0.05))

      ${''/* ── F.Cu SMD pads ─────────────────────────────────────── */}
      ${''/* Pad height extended from 1.8 to 3.0 mm; center shifted  */}
      ${''/* from -2.225 to -2.825 so the connector-side inner edge  */}
      ${''/* stays at y = -1.325, and the exposed land grows from     */}
      ${''/* ~0.9 mm to ~2.0 mm for comfortable drag soldering.       */}
      ${''/* A1  — GND (outer left)  */}
      (pad "A1"  smd rect (at -3.29 -2.825 ${p.r}) (size 0.3 3.0) (layers "F.Cu" "F.Paste" "F.Mask") ${gnd})
      ${''/* B12 — GND (inner left)  */}
      (pad "B12" smd rect (at -2.99 -2.825 ${p.r}) (size 0.3 3.0) (layers "F.Cu" "F.Paste" "F.Mask") ${gnd})
      ${''/* B1  — GND (inner right) */}
      (pad "B1"  smd rect (at  2.99 -2.825 ${p.r}) (size 0.3 3.0) (layers "F.Cu" "F.Paste" "F.Mask") ${gnd})
      ${''/* A12 — GND (outer right) */}
      (pad "A12" smd rect (at  3.29 -2.825 ${p.r}) (size 0.3 3.0) (layers "F.Cu" "F.Paste" "F.Mask") ${gnd})
      ${''/* B7  — D- (USB-C spec: B7 = D2−, faces A6 across the slot) */}
      (pad "B7"  smd rect (at -0.75 -2.825 ${p.r}) (size 0.3 3.0) (layers "F.Cu" "F.Paste" "F.Mask") ${dm})
      ${''/* A6  — D+ (USB-C spec: A6 = D1+) */}
      (pad "A6"  smd rect (at -0.25 -2.825 ${p.r}) (size 0.3 3.0) (layers "F.Cu" "F.Paste" "F.Mask") ${dp})
      ${''/* A7  — D- (USB-C spec: A7 = D1−) */}
      (pad "A7"  smd rect (at  0.25 -2.825 ${p.r}) (size 0.3 3.0) (layers "F.Cu" "F.Paste" "F.Mask") ${dm})
      ${''/* B6  — D+ (USB-C spec: B6 = D2+, faces A7 across the slot) */}
      (pad "B6"  smd rect (at  0.75 -2.825 ${p.r}) (size 0.3 3.0) (layers "F.Cu" "F.Paste" "F.Mask") ${dp})

      ${''/* ── B.Cu SMD pads (mirrored) ──────────────────────────── */}
      ${''/* B1  — GND */}
      (pad "B1"  smd rect (at -2.99 -2.825 ${p.r}) (size 0.3 3.0) (layers "B.Cu" "B.Paste" "B.Mask") ${gnd})
      ${''/* A12 — GND */}
      (pad "A12" smd rect (at -3.29 -2.825 ${p.r}) (size 0.3 3.0) (layers "B.Cu" "B.Paste" "B.Mask") ${gnd})
      ${''/* A1  — GND */}
      (pad "A1"  smd rect (at  3.29 -2.825 ${p.r}) (size 0.3 3.0) (layers "B.Cu" "B.Paste" "B.Mask") ${gnd})
      ${''/* B12 — GND */}
      (pad "B12" smd rect (at  2.99 -2.825 ${p.r}) (size 0.3 3.0) (layers "B.Cu" "B.Paste" "B.Mask") ${gnd})
      ${''/* A6  — D+ (USB-C spec: A6 = D1+) */}
      (pad "A6"  smd rect (at  0.25 -2.825 ${p.r}) (size 0.3 3.0) (layers "B.Cu" "B.Paste" "B.Mask") ${dp})
      ${''/* B6  — D+ (USB-C spec: B6 = D2+) */}
      (pad "B6"  smd rect (at  0.75 -2.825 ${p.r}) (size 0.3 3.0) (layers "B.Cu" "B.Paste" "B.Mask") ${dp})
      ${''/* A7  — D- (USB-C spec: A7 = D1−) */}
      (pad "A7"  smd rect (at -0.25 -2.825 ${p.r}) (size 0.3 3.0) (layers "B.Cu" "B.Paste" "B.Mask") ${dm})
      ${''/* B7  — D- (USB-C spec: B7 = D2−) */}
      (pad "B7"  smd rect (at -0.75 -2.825 ${p.r}) (size 0.3 3.0) (layers "B.Cu" "B.Paste" "B.Mask") ${dm})

      ${''/* ── Shield / mounting holes (GND) ────────────────────── */}
      ${''/* Oval thru-hole pads: both size and drill are oval,      */}
      ${''/* so ${p.r} is needed on both the pad at and the drill.   */}
      ${''/* Shield / mounting holes (GND)                                       */}
      ${''/* Measured tab cross-section: 0.30 mm wide × 1.21 mm (rear) or       */}
      ${''/* 0.80 mm (front). Drills add ~0.05 mm clearance per side.            */}
      ${''/* Pad = drill + 0.20 mm annular ring per side.                        */}
      (pad "S1" thru_hole oval (at  5.62  2.58 ${p.r}) (size 0.8 1.7) (drill oval 0.4 1.3) (layers "*.Cu" "*.Mask") ${gnd})
      (pad "S1" thru_hole oval (at  5.62 -1.42 ${p.r}) (size 0.8 1.3) (drill oval 0.4 0.9) (layers "*.Cu" "*.Mask") ${gnd})
      (pad "S1" thru_hole oval (at -5.62  2.58 ${p.r}) (size 0.8 1.7) (drill oval 0.4 1.3) (layers "*.Cu" "*.Mask") ${gnd})
      (pad "S1" thru_hole oval (at -5.62 -1.42 ${p.r}) (size 0.8 1.3) (drill oval 0.4 0.9) (layers "*.Cu" "*.Mask") ${gnd})
    )
    `
  }
}
